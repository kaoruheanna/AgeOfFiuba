...
TipoConfig::TipoConfig(YAML::Node nodo): TipoConfig(){
	...
	if(this->nombre == ""){
		Log().Get(logERROR) << "Nombre no puede ser null para tipo TipoConfig";
	}
	if(this->imagen == ""){
		Log().Get(logERROR) << "Imagen no puede ser null para tipo TipoConfig";
	}
}
...
int TipoConfig::getIntAttribute(YAML::Node nodo, string attributeName, int defaultValue) {
	if(!nodo[attributeName] || nodo[attributeName].IsNull() || !nodo[attributeName].IsDefined()){
		Log().Get(logDEBUG) << attributeName << " no esta definido para este tipo. Usando valor default: " << defaultValue;
	} else if(!nodo[attributeName].IsScalar()){
		Log().Get(logDEBUG) << attributeName << " no es del tipo int. Usando valor default: " << defaultValue;
	} else {
		try{
			return nodo[attributeName].as<int>();
		} catch(YAML::RepresentationException& error){
			Log().Get(logDEBUG) << attributeName << " no se puede castear a int. Error YAML: " << error.msg << ". Usando valor default: " << defaultValue;
		}
	}
	return defaultValue;
}
string TipoConfig::getStringAttribute(YAML::Node nodo, string attributeName, string defaultValue){
	if(!nodo[attributeName] || nodo[attributeName].IsNull() || !nodo[attributeName].IsDefined()){
		...
		try{
			...
			Log().Get(logDEBUG) << attributeName << " no se puede castear a string. Error YAML: " << error.msg << ". Usando valor default: " << defaultValue;
		}
	}
	return defaultValue;
}
